...
import { AdditionalServiceView, BookingService, BookingView } from '~/services';
// import { Cur, CustomCheckbox, getUrlRoute, Naira, Rating } from 'ui';

const PaymentPage = ({ bookings }: { bookings: BookingView }) => {
  ...
  const [selectedAddon] = useState<AdditionalServiceView[]>(
  ...
  const checkoutBooking = async (id: string) => {
    ...
    try {
      const result = await BookingService.checkout({
        id,
        frontEndBaseUrl: `/payment/validate`,
      });
      if (result.status) {
